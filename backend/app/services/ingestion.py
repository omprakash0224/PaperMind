...
from app.services.vectorstore import get_langchain_vectorstore, get_qdrant_client
...
# ── Rate-limit config ─────────────────────────────────────────────────────────
BATCH_SIZE     = 5      # chunks per embed call (free tier: 5 RPM)
BATCH_DELAY    = 12     # seconds between batches (60s / 5 RPM = 12s)
MAX_RETRIES    = 5
RETRY_BASE     = 15     # seconds — doubles on each retry
...
def _add_batch_with_retry(vectorstore, batch: list[Document]) -> None:
    """
    Add a batch of documents with exponential backoff on 429 rate-limit errors.
...
def ingest_document(file_path: str, filename: str, *, user_id: str) -> dict:
    ...
    1. Hash check (per user) → skip if duplicate
    2. Parse PDF/DOCX into page-level text blocks
    3. Split into chunks with RecursiveCharacterTextSplitter (tiktoken-based)
    4. Embed via GoogleGenerativeAIEmbeddings and store in Qdrant
    ...
    try:
        ...
        if not langchain_docs:
            raise ValueError(f"Document '{filename}' produced no text chunks after splitting.")

        logger.info("Split '%s' into %d chunks (user=%s).", filename, len(langchain_docs), user_id)

        # ── 4. Embed and store in Qdrant ──────────────────────────────────────────
        embeddings = GoogleGenerativeAIEmbeddings(
            model=settings.EMBEDDING_MODEL,
            google_api_key=settings.GOOGLE_API_KEY,
            # Output 768 dims — recommended by Google, matches our Qdrant collection
        ...
        logger.info("Ingestion complete for '%s': %d chunks stored.", filename, len(langchain_docs))
